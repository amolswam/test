...
--1.Blocking
--select [Session ID],[Blocked By],[Head Blocker],login,DatabaseName,[Task State],statement_text,Application,[Wait Type],[Total CPU (ms)],[Host Name],[Net Address]
--from #temp where [Blocked By] > 0 and [Blocked By] <> [Session ID] 
--union
--select [Session ID],[Blocked By],[Head Blocker],login,DatabaseName,[Task State],statement_text,Application,[Wait Type],[Total CPU (ms)],[Host Name],[Net Address]
--from #temp where [Session ID] in (select T.[Blocked By] from #temp T where T.[Blocked By] <> T.[Session ID] ) 

--2.CPU wise
--select DISTINCT 'dbcc inputbuffer(' + cast([Session ID] as nvarchar)+ ')',[Session ID],[Blocked By],[Head Blocker],Login,DatabaseName,[Task State],command,statement_text,Application,[Wait Type],[Total CPU (ms)],[Host Name],[Net Address]
--from #temp
--ORDER BY [Total CPU (ms)] desc 

--3. select Database
--select [Session ID],[Blocked By],[Head Blocker],DatabaseName,Login,[Task State],command,statement_text,Application,[Wait Type],[Total CPU (ms)],[Host Name],[Net Address]
--from #temp
--where DatabaseName in ('')


--4. For restart 
select distinct [Session ID],[Blocked By],Login,[Head Blocker],DatabaseName,[Task State],command,statement_text,Application,[Wait Type],[Total CPU (ms)],[Host Name],[Net Address]
from #temp
where [Task State] <> '' and Command <> 'BRKR TASK'
--order by Command
